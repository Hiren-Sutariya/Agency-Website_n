...
function initHeroAnimation() {
    ...
    const count = 8000; // Increased for dense, solid look
    const radius = 6.0;
    ...
    function animateHero() {
        ...
        if (heroCloud) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                // Improve performance by pre-calculating distance components
                const dx = tarX - localMouse.x;
                const dy = tarY - localMouse.y;
                // We mainly interact via X/Y plane for the "look", Z is depth
                ...
                if (distSq < interactRadiusSq) {
                    ...
                    // Gaussian curve for smooth denting
                    // A value from 0 to 1 based on proximity (1 at center, 0 at edge)
                    const influence = Math.exp(-distSq * 0.15);

                    // Push INWARD (towards 0,0,0) based on influence
                    // But we want it to look like the mouse is physically pressing it.
                    // So we push Z away from camera (negative Z) or towards center?
                    // User ref shows a "dent". A dent means points move AWAY from the viewer / interaction source.
                    // Since camera is at +Z, "away" is -Z.

                    const dentStrength = -2.5 * influence; // Depth of poke

                    // Displace along the z-axis of the interaction point perspective? 
                    // Simple radial displacement towards center is 'implosion'.
                    // We want 'dent'. 

                    // Let's displace towards the sphere center based on mouse proximity
